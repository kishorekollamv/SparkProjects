...
object DataLake extends GetDataTrait with WriteDataTrait{
...
val dfemployees= getRdbmsData(sqlc,"empoffice",customquery,"employeeNumber",s"$confile",s"$upperbound".toLong,true) 
...
val dfoffices= getRdbmsData(sqlc,"empoffice","offices","",s"$confile",s"$upperbound".toLong,false) 
...
val dfcustpayments= getRdbmsData(sqlc,"custpayments",custpaymentsquery,"customernumber",s"$confile",5000.toLong,true) 
...
val dforderprod= getRdbmsData(sqlc,"ordersproducts",orderprodquery,"customernumber",s"$confile",1000.toLong,true) 
...
spark.sql("""insert overwrite table retail_curated.custdetstgcured select coalesce(customernumber,0),contactfirstname, 
  ...
  spark.sql("""create external table retail_curated.custdetpartbuckext	(customernumber STRING, customername STRING, contactfullname string, address struct<addressLine1:string,city:string,state:string,postalCode:bigint,country:string,phone:bigint>,creditlimit float,checknum string,checkamt int) 
partitioned by (paymentdate date) 
row format delimited fields terminated by '~' collection items terminated by '$' 
stored as textfile location 'hdfs://localhost:54310/user/hduser/custorders/custdetpartbuckext'""");
...
row format delimited fields terminated by '~' collection items terminated by '$' 
stored as textfile location 'hdfs://localhost:54310/user/hduser/custorders/orddetpartbuckext'""")
...
row format delimited fields terminated by ','
location 'hdfs://localhost:54310/user/hduser/retailorders/dimorders/'""")

spark.sql("""load data local inpath '/home/hduser/retailorders/orders_rate.csv' overwrite into table retail_discovery.dim_order_rate""")
...
spark.sql("""load data local inpath '/home/hduser/retailorders/cust_visits.csv' overwrite into table retailstg. cust_navigation""")
...
row format delimited fields terminated by ',' 
location 'hdfs://localhost:54310/user/hduser/custnavigation/'""")
...
writeFile(report1.coalesce(1),"json","hdfs://localhost:54310/user/hduser/retail_discovery/first_last_page",false,"");
...
writeFile(mostvisit.coalesce(1),"json","hdfs://localhost:54310/user/hduser/retail_discovery/mostvisited",false,"");
...
spark.sql("""create external table if not exists retail_discovery.cust_frustration_level 
  ...
  row format delimited fields terminated by ',' location 'hdfs://localhost:54310/user/hduser/custmartfrustration/'""")
